...
function isLongURL(){
    ...
    if(url.length<54){
        console.log("NP");
        return -1; //Safe due to URL length being within given limit
    } 
        ...
        console.log("P");
        return 1; //Malicious due to URL length exceeding given limit
    }
}
function isTinyURL(){
    var url = window.location.href;    
    if(url.length>20){
        console.log("NP");
        return -1; //Safe due to URL length surpassing given limit
    } 
    else{
        console.log("P");
        return 1; //Malicious due to URL length falling short of given limit
    }
}
...
function isRedirectingURL(){
    ...
    if(url.search(srch)==5 && reg1.exec(url)!=null && (url.substring(7)).match(srch)==null){
        console.log("NP");
        return -1;  //if URL is redirecting towards an https site mark as safe
    }
        ...
        console.log("P");
        return 1;   //if URL is redirecting to an http site mark as malicious
    }
}
function isHypenURL(){
    var reg = /[a-zA-Z]\//;
    ...
    if(((url.substring(0,url.search(reg)+1)).match(srch))==null){
        console.log("NP");
        return -1;  //if hyphen is not present mark as safe
    }    
    else{
        console.log("P");
        return 1;  //if hyphen is present mark as malicious
    }
}
...
function isIllegalHttpsURL(){
    ...
    if(((url.substring(url.search(srch1))).match(srch2))==null){
        console.log("NP");
        return -1;  //if https is found in URL then mark safe
    }    
    else{
        console.log("P");
        return 1; //if http is present mark as malicious
    }
}
function isImgFromDifferentDomain(){
	...
	if(((totalCount-identicalCount)/totalCount)<0.22){
        console.log("NP");
        return -1;   //if domain is same mark safe
    } 
	else if((((totalCount-identicalCount)/totalCount)>=0.22) && (((totalCount-identicalCount)/totalCount)<=0.61)){
        console.log("Maybe");44
        return 0;
    } 	
    else{
        console.log("P");
        return 1;    //if domain is different mark malicious
    }
}
...
function isMailToAvailable(){
    if(document.querySelectorAll('a[href^=mailto]').length<=0){
        console.log("NP");
        return -1;  //if there is a link available for mailing 
    } 	
    else{
        console.log("P");
        return 1;   
...
function isIframePresent(){
    if(document.querySelectorAll('iframe').length<=0){
        console.log("NP");
        return -1;  //if Iframes are present mark as malicious since scripts can be hidden in Iframes
    }
    else{
        console.log("P");
        return 1; 
